...
class StoreListingRepository @Inject constructor(private val storeApiServices: StoreApiServices) {
    private val storeListResponseMutableLiveData = MutableLiveData<NetworkResult<GetStoreListResponse>>()
    ...
    private fun handleResponse(response: Response<GetStoreListResponse>) {
        if (response.isSuccessful && response.body() != null) {
            ...
            storeListResponseMutableLiveData.postValue(NetworkResult.Error(ErrorModel("Exception", 10)))
        }
    }
}